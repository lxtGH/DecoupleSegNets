...
from network.nn.operators import conv_bn_relu, conv_sigmoid, _AtrousSpatialPyramidPoolingModule
...
class DeepWV3PlusGFF(nn.Module):

    def __init__(self, num_classes, trunk='WideResnet38', criterion=None):

        super(DeepWV3PlusGFF, self).__init__()
        self.criterion = criterion
        logging.info("Trunk: %s", trunk)
        ...
        if criterion is not None:
            try:
                checkpoint = torch.load('./pretrained_models/wider_resnet38.pth.tar', map_location='cpu')["state_dict"]
                wide_resnet.load_state_dict(checkpoint)
                del checkpoint
            except:
                print("Please download the ImageNet weights of WideResNet38 in our repo to ./pretrained_models/wider_resnet38.pth.tar.")
        ...
        self.gff_head = WiderResNetGFFHead(num_classes, norm_layer=Norm2d)
...
class WiderResNetGFFHead(nn.Module):

    def __init__(self, num_classes, norm_layer=nn.BatchNorm2d):
        super(WiderResNetGFFHead, self).__init__()
